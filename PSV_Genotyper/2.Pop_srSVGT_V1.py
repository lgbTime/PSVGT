...
def split_region(start, end):
    # Calculate the step size for dividing the region into 6 equal parts
    step = (end - start) / 7
    # Generate the five split points
    points = [start + i * step for i in range(8)]  # 6 points (start + 5 splits)
    del points[0]
    del points[-1]
    return points
...
def process_sv(sv_line, opened_sam, name, min_maq, shift):
    ...
    if "DEL" in info[5]:
        ...
        if genotype[0] != "1/1":
            ...
            print(local_5counts)
            if local_5counts.count(0) >= 2: ##  2 regions count 0 reads
                genotype[0] = "1/1"
...
def srSVGT(sv_table, mapf, name, outdir, min_maq, shift):
    ...
    for sv_line in tqdm(set(sv_lines[1:])):
        result = process_sv(sv_line, mapf, name, min_maq, shift)
        output_lines.append(result)
    output_file_path = f"{outdir}/2_tmp_{name}_bpgenotype.txt"
    with open(output_file_path, "w") as output_file:
        output_file.write(header_line + "\n")
        for line in set(output_lines):
...
if __name__ == "__main__":
    ...
    IN.add_argument("-i", dest="sv_info",required=True, help="InDel record table can be generated by PSVGT step 1")
    IN.add_argument("-m", dest="maq", default=1,type=int, help="the mini mapping quality of the contigs, range from 0-60, default is 45")
    IN.add_argument("-s", dest="shift",default=30,type=int, help="breakpoints may shifting, if the shifting range is expand, the clip reads ratio to call genotype should be smaller, please check sub_svGT ratio to determine the genotype")
