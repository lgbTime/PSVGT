...
def process_sv(sv_line, opened_sam, name, min_maq, shift, span):
    sampleID = name
...
def process_chromosome(chromosome_lines, mapf_path, name, min_maq, shift, span):
    output_lines = []
    try:
        opened_sam = pysam.AlignmentFile(mapf_path)
        for sv_line in chromosome_lines:
            result = process_sv(sv_line, opened_sam, name, min_maq, shift, span)
...
def srSVGT(sv_table, mapf, name, outdir, min_maq, shift, span):
    ...
    tasks = [(chromosome_lines, mapf, name, min_maq, shift, span) for chromosome_lines in chromosome_groups.values()]
...
if __name__ == "__main__":
    ...
    IN.add_argument("-m", dest="maq", default=1, type=int,
    ...
    start_t = time()
    srSVGT(args.sv_info, args.mapf, args.ACC, args.dir, args.maq, args.shift, args.span)
    end_t = time()
