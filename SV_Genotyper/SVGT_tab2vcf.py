...
##INFO=<ID=SV_END,Number=1,Type=Integer,Description="End position of the structural variant">
...
#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT\t{basename(tab)}"""
        ...
        for line in lines:
            ...
            #outline = f"{chrom}\t{sv_start}\t{ID}\tN\t<{sv_type}>\t.\tPASS\tCHR={chrom};SV_START={sv_start};SV_END={sv_end};SVLEN={svlen};method=PSVGT;{maps};{rate};SVTYPE={sv_type}\tGT\t{gt}"
            outline = f"{chrom}\t{sv_start}\t{ID}\tN\t<{sv_type}>\t.\tPASS\tCHR={chrom};SV_START={sv_start};SV_END={sv_end};SVLEN={svlen};method=PSVGT;{maps};{rate};SVTYPE={sv_type}\tGT\t{gt}"
