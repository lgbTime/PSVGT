...
def match_bam_file(sample_header, bam_dir):
    ...
    """
    # Clean the header string to isolate the root accession name
    prefix = sample_header.split(".")[0]
    
    # Common variations based on your file listings
    possible_names = [
...
def main():
    ...
    parser.add_argument("-b", "--bam-dir", required=True, help="Directory containing your sample *_sorted.bam files")
    ...
    parser.add_argument("-ref_rate", dest="ref_rate", type=float, default=0.30, help="Reference (0/0) ratio threshold")
    parser.add_argument("-shift", dest="shift", type=int, default=25, help="Breakpoint shift range")
    ...
    print(f"[*] Mapping columns to files inside BAM directory: '{args.bam_dir}'...")
    
    # Build open file handle registry for all matching BAM accessions
    bam_handles = {}
    missing_samples = []
    
    for idx, sample in enumerate(sample_headers):
    ...
    print("[*] Commencing read-backed verification across calls (INS > 100bp only)...")
    ...
    for data in tqdm(lines[1:], desc="Processing structural profiles"):
        ...
        is_target_ins = (sv_type == "INS" and sv_size > 100)
