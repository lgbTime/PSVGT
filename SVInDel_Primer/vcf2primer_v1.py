...
def my_primer(info, seq, seqid, seq_args_left, seq_args_right, global_args, sv_size):
    ...
    if "N" not in seq: 
        if len(primers_L) >0  and len(primers_R) > 0:
            ...
            outinfo = "\t".join(info)
            print(f"{outinfo}\t{seqid}\t{seq}\tForward primers: {set(pos_f)}\tReverse primers: {set(pos_r)}\tMiniPCR_Pair_Primer:{(best_forward_primer,best_reverse_primer)}\t{miniPCR1}\t{miniPCR2}")
...
def is_valid_dna_sequence(sequence):
    ...
    valid_nucleotides = {'A','a', 'G','g', 'C','c', 'T','t', 'N'}
